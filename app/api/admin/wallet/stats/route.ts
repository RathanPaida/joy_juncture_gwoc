...
export async function GET(req: NextRequest) {
  try {
    ...
    const totalPointsIssued =
      pointsIssuedResult.length > 0 ? pointsIssuedResult[0].total : 0;
    ...
    const totalPointsRedeemed =
      pointsRedeemedResult.length > 0
        ? Math.abs(pointsRedeemedResult[0].total)
        : 0;
    ...
    const pointsInCirculation = await db
      .collection("users")
      .aggregate([{ $group: { _id: null, total: { $sum: "$totalPoints" } } }])
      .toArray();
    const totalPointsInCirculation =
      pointsInCirculation.length > 0 ? pointsInCirculation[0].total : 0;
