  ...
  const handleStartGame = async () => {
    ...
    try {
      ...
      if (typeof user.getIdToken === 'function') {
        ...
        // If you're using session-based auth without tokens
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        const mockGameData = createMockGameData(game.id, selectedDifficulty);
        sessionStorage.setItem(`currentGame_${game.id}`, JSON.stringify(mockGameData));
        window.location.href = `/games/${game.id}?gameId=${mockGameData.gameId}`;
      } else {
    ...
    switch (gameType) {
      case 'sudoku':
        return {
          ...
          hints: [],
          initial: Array(9).fill(0).map(() => Array(9).fill(0))
        };
      case 'word-guesser':
        return {
          ...baseData,
          puzzle: 'ELHLOWORLD',
          hint: 'A common programming greeting',
          solution: 'HELLOWORLD'
        };
      case 'crossword':
        return {
          ...
          clues: [
            ...
            { number: 2, direction: 'down', clue: 'Our planet' }
          ],
          size: 5
        };
      default:
