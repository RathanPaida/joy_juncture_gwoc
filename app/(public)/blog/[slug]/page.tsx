...
import {
  FaArrowLeft, FaCalendar, FaUser, FaClock, FaTag, FaStar
} from 'react-icons/fa';
  ...
  const fetchBlog = async () => {
    ...
    try {
      ...
      if (listResponse.ok) {
        ...
        console.log('📋 Available blogs:', listData.blogs?.map((b: Blog) => ({
          title: b.title,
          slug: b.slug,
          status: b.status
        })));
        ...
        if (matchingBlog) {
          ...
          if (matchingBlog.status === 'published' || 
    ...
    return (
      <div className="blog-detail-page">
        <div className="not-found">
          ...
          <p>{error || 'The article you\'re looking for doesn\'t exist or may have been removed.'}</p>
          ...
          <div style={{ marginTop: '20px', display: 'flex', gap: '10px', justifyContent: 'center' }}>
            ...
            <button onClick={() => window.location.reload()} style={{ background: '#6366f1' }}>
              🔄 Retry
            </button>
          </div>
        </div>
        ...
        <article className="blog-article">
          <header>
            ...
            <div className="blog-meta">
              ...
              <span className="meta-item">
                <FaCalendar /> {new Date(blog.publishedDate || blog.createdAt!).toLocaleDateString()}
              </span>
              {blog.readTime && (
                <span className="meta-item">
