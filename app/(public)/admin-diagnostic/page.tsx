...
export default function RewardDeleteDiagnostic() {
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '50px auto', padding: '30px', fontFamily: 'system-ui' }}>
      <h1 style={{ fontSize: '32px', marginBottom: '10px' }}>🔍 Reward Delete Diagnostic</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px' }}>
          ...
          {rewards.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              {rewards.map((reward) => (
                ...
                >
                  <h3 style={{ marginTop: 0, marginBottom: '10px' }}>{reward.name}</h3>
                  <p style={{ fontSize: '12px', color: '#666', marginBottom: '10px', fontFamily: 'monospace' }}>
                    ID: {reward._id}
                  </p>
        ...
        <div>
          ...
          >
            {logs.length === 0 ? (
              <div style={{ color: '#666' }}>Click "Test Delete" on any reward...</div>
            ) : (
              logs.map((log, i) => (
      ...
      >
        ...
        <ul style={{ lineHeight: '1.8' }}>
          <li><strong>If deletedCount = 0:</strong> The ID doesn't exist in MongoDB</li>
          <li><strong>If ID looks weird:</strong> There might be extra characters or formatting issues</li>
          <li><strong>If ID length is wrong:</strong> MongoDB ObjectIDs should be 24 characters</li>
          <li><strong>Copy the logs and send them to me</strong> so I can see exactly what's happening</li>
        </ul>
      </div>
    </div>
  );
