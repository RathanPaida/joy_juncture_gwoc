...
const AdminDebugPage: React.FC = () => {
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        <h1 style={{ color: '#FF8C00', marginBottom: '2rem' }}>Admin Access Debug</h1>
          ...
          <>
            <div style={styles.section}>
              ...
              <div style={styles.infoGrid}>
                ...
                <div><strong>Display Name:</strong> {debugInfo.userDisplayName}</div>
                <div><strong>Firebase UID:</strong> {debugInfo.userUid}</div>
                <div><strong>Token Obtained:</strong> {debugInfo.tokenObtained ? '✅' : '❌'}</div>
              </div>
            </div>
            ...
            {debugInfo.mongoUser && (
              <div style={styles.section}>
                ...
                <div style={styles.infoGrid}>
                  <div><strong>Email:</strong> {debugInfo.mongoUser.email}</div>
                  <div><strong>Role:</strong> <span style={{
                    color: debugInfo.mongoUser.role === 'admin' || debugInfo.mongoUser.role === 'super_admin' 
                      ? '#2ECC71' : '#E74C3C'
                  }}>{debugInfo.mongoUser.role}</span></div>
                  <div><strong>Firebase UID:</strong> {debugInfo.mongoUser.firebaseUid || 'NOT SET'}</div>
                  <div><strong>Has Firebase UID:</strong> {debugInfo.mongoUser.hasFirebaseUid ? '✅' : '❌'}</div>
                </div>
              </div>
            )}
              ...
              <div style={styles.testResults}>
                <div style={styles.testItem}>
                  ...
                  <span style={{ 
                    ...
                    {debugInfo.walletApiStatus} {debugInfo.walletApiOk ? '✅' : '❌'}
                  </span>
                ...
                <div style={styles.testItem}>
                  ...
                  <span style={{ 
                    ...
                    {debugInfo.adminCheckStatus} {debugInfo.adminCheckOk ? '✅' : '❌'}
                  </span>
            ...
            {debugInfo.routeChecks && (
              <div style={styles.section}>
                ...
                <div style={styles.routeChecks}>
                  {Object.entries(debugInfo.routeChecks).map(([route, check]: [string, any]) => (
              ...
              <div style={styles.troubleshoot}>
                {!debugInfo.mongoUser && (
                  <div style={styles.issue}>
                    ...
                    <p>Solution: Visit /walletandpoints to create your wallet first</p>
                  </div>
                )}
                ...
                {debugInfo.mongoUser && !debugInfo.mongoUser.hasFirebaseUid && (
                  <div style={styles.issue}>
                    ...
                    <p>Solution: Visit /walletandpoints once to link your Firebase account</p>
                  </div>
                )}
                
                {debugInfo.mongoUser && debugInfo.mongoUser.role === 'viewer' && (
                  <div style={styles.issue}>
                ...
                {debugInfo.routeChecks && 
                  Object.values(debugInfo.routeChecks).some((r: any) => !r.exists) && (
                  <div style={styles.issue}>
