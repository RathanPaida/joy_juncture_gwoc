...
import { Trash2, Edit, Plus, X, Save, Package, ArrowLeft, LogOut } from 'lucide-react';
...
export default function AdminPackagesPage() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      console.log("🔐 Admin Packages: Auth state changed:", firebaseUser?.email);
      ...
      if (data.success && Array.isArray(data.data)) {
        ...
        const transformedPackages = data.data.map((pkg: any, index: number) => {
          ...
          return {
            ...packageWithId,
            includes: {
              food: Array.isArray(pkg.includes?.food) ? pkg.includes.food.filter(Boolean) : [],
              planning: Array.isArray(pkg.includes?.planning) ? pkg.includes.planning.filter(Boolean) : [],
              sound: Array.isArray(pkg.includes?.sound) ? pkg.includes.sound.filter(Boolean) : [],
              photography: Array.isArray(pkg.includes?.photography) ? pkg.includes.photography.filter(Boolean) : [],
              games: Array.isArray(pkg.includes?.games) ? pkg.includes.games.filter(Boolean) : []
            }
          };
        });
  ...
  const openCreateModal = () => {
    ...
    setPackageForm({
      ...
      color: '#ff8c00',
      category: '',
      includes: { food: [''], planning: [''], sound: [''], photography: [''], games: [''] }
    });
    setIsModalOpen(true);
  };
  ...
  const openEditModal = (pkg: GamePackage) => {
    ...
    setPackageForm({
      ...
      includes: {
        ...
        photography: pkg.includes?.photography?.length ? pkg.includes.photography : [''],
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const filteredIncludes = {
        ...
        planning: packageForm.includes.planning.filter(item => item.trim() !== ''),
        sound: packageForm.includes.sound.filter(item => item.trim() !== ''),
        photography: packageForm.includes.photography.filter(item => item.trim() !== ''),
      ...
      if (data.success) {
        ...
        setError(data.error || `Failed to ${editingPackage ? "update" : "add"} package`);
      }
    } catch (err: any) {
      console.error(err);
  ...
  const removeIncludeItem = (field: keyof typeof packageForm.includes, index: number) => {
    setPackageForm(prev => ({ 
      ...prev, 
      includes: { 
        ...prev.includes, 
  ...
  const updateIncludeItem = (field: keyof typeof packageForm.includes, index: number, value: string) => {
    setPackageForm(prev => ({
      ...prev,
      includes: { 
        ...prev.includes, 
    ...
    return (
      <div style={{ minHeight: "100vh", background: `linear-gradient(135deg, ${colors.dark}, ${colors.light})`, color: colors.text, fontFamily: "system-ui,sans-serif", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ background: colors.background, borderRadius: "12px", padding: "3rem", textAlign: "center", border: `2px solid ${colors.primary}`, maxWidth: "500px", width: "90%" }}>
          <Package size={64} style={{ color: colors.primary, marginBottom: "1rem" }} />
    ...
    return (
      <div style={{ minHeight: "100vh", background: `linear-gradient(135deg, ${colors.dark}, ${colors.light})`, color: colors.text, fontFamily: "system-ui,sans-serif", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center" }}>
          <Package size={48} className="animate-spin" style={{ color: colors.primary, marginBottom: "1rem" }} />
  ...
  return (
    <div style={{ minHeight: '100vh', background: `linear-gradient(135deg, ${colors.dark} 0%, ${colors.light} 100%)`, color: colors.text, fontFamily: 'system-ui, sans-serif' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginTop: '0.25rem' }}>
                <p style={{ color: colors.textLight, fontSize: '0.9rem' }}>Total Packages: {packages.length}</p>
                <p style={{ color: colors.textLight, fontSize: '0.9rem' }}>Logged in as: {user?.email}</p>
              </div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <button onClick={refreshPackages} style={{ background: colors.light, color: colors.text, border: `1px solid ${colors.border}`, borderRadius: '8px', padding: '10px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.9rem' }}>
              🔄 Refresh
            </button>
      ...
      <div style={{ padding: '2rem', maxWidth: '1400px', margin: '0 auto' }}>
        {isLoading ? (
          <div style={{ textAlign: 'center', padding: '4rem', color: colors.textLight }}>Loading packages...</div>
        ) : packages.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '4rem' }}>
            <Package size={64} style={{ color: colors.textLight, marginBottom: '1rem' }} />
            <h3 style={{ color: colors.text, marginBottom: '0.5rem' }}>No packages yet</h3>
            <p style={{ color: colors.textLight }}>Click "Add New Package" to create your first package</p>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '2rem' }}>
            {packages.map((pkg, index) => {
              ...
              return (
                ...
                >
                  <div style={{ background: pkg.color, padding: '1.5rem', textAlign: 'center' }}>
                    <h3 style={{ color: colors.dark, fontSize: '1.3rem', marginBottom: '0.5rem', fontWeight: 800 }}>{pkg.name}</h3>
                    <div style={{ fontSize: '2rem', fontWeight: 'bold', color: colors.dark, marginBottom: '0.5rem' }}>{pkg.price}</div>
                    <div style={{ fontSize: '0.9rem', color: colors.dark, fontWeight: 600 }}>{pkg.duration} • {pkg.guestRange}</div>
                  </div>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '2rem' }}>
          <div style={{ background: colors.background, borderRadius: '12px', padding: '2rem', maxWidth: '700px', width: '100%', maxHeight: '85vh', overflow: 'auto', border: `2px solid ${colors.primary}` }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem', alignItems: 'center' }}>
              <h2 style={{ color: colors.text, fontSize: '1.5rem', fontWeight: 700 }}>{editingPackage ? 'Edit Package' : 'Add New Package'}</h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              {/* Basic Info */}
              <div>
                <label style={{ display: 'block', color: colors.text, marginBottom: '0.5rem', fontWeight: 600 }}>Name *</label>
                <input 
                  type="text" 
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  <label style={{ display: 'block', color: colors.text, marginBottom: '0.5rem', fontWeight: 600 }}>Price *</label>
                  <input 
                    type="text" 
                    value={packageForm.price} 
                    onChange={e => setPackageForm({ ...packageForm, price: e.target.value })} 
                    placeholder="₹ 1499" 
                    style={{ width: '100%', padding: '12px', background: colors.light, border: `1px solid ${colors.border}`, borderRadius: '8px', color: colors.text }} 
                  />
                </div>
                <div>
              ...
              <div>
                <label style={{ display: 'block', color: colors.text, marginBottom: '0.5rem', fontWeight: 600 }}>Guest Range *</label>
                <input 
                  type="text" 
                  value={packageForm.guestRange} 
                  onChange={e => setPackageForm({ ...packageForm, guestRange: e.target.value })} 
              ...
              <div>
                <label style={{ display: 'block', color: colors.text, marginBottom: '0.5rem', fontWeight: 600 }}>Best For</label>
                <input 
                  type="text" 
              ...
              <div>
                <label style={{ display: 'block', color: colors.text, marginBottom: '0.5rem', fontWeight: 600 }}>Category *</label>
                <select 
              ...
              <div>
                <label style={{ display: 'block', color: colors.text, marginBottom: '0.5rem', fontWeight: 600 }}>Color</label>
                <select 
              ...
              {/* Includes Sections */}
              {(['food', 'planning', 'sound', 'photography', 'games'] as const).map(section => (
                <div key={section}>
                  <label style={{ display: 'block', color: colors.text, marginBottom: '0.5rem', fontWeight: 600 }}>
                    {section.charAt(0).toUpperCase() + section.slice(1)} Items
                  </label>
                  {packageForm.includes[section].map((item, idx) => (
                    <div key={`${section}-${idx}`} style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem' }}>
                      <input 
                        type="text" 
                        value={item} 
                        onChange={e => updateIncludeItem(section, idx, e.target.value)} 
                        placeholder={`Add ${section} item`} 
                        style={{ flex: 1, padding: '10px', background: colors.light, border: `1px solid ${colors.border}`, borderRadius: '8px', color: colors.text }} 
                      />
                      {packageForm.includes[section].length > 1 && (
                        <button 
                          type="button" 
                          onClick={() => removeIncludeItem(section, idx)} 
                          style={{ background: colors.danger, color: 'white', border: 'none', borderRadius: '6px', padding: '10px', cursor: 'pointer' }}
                        >
                          <X size={16} />
                        </button>
                      )}
                  ...
                  <button 
                    ...
                    style={{ background: colors.success, color: 'white', border: 'none', borderRadius: '6px', padding: '8px 12px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.5rem' }}
                  >
              ...
              <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                <button 
                  onClick={closeModal} 
                  style={{ flex: 1, background: colors.border, color: colors.text, border: 'none', borderRadius: '8px', padding: '14px', cursor: 'pointer', fontWeight: 600, fontSize: '1rem' }}
                >
                  Cancel
                </button>
                <button 
                  ...
                  style={{ 
                    flex: 1, 
                    background: isSubmitting ? colors.textLight : `linear-gradient(135deg, ${colors.primary}, ${colors.secondary})`, 
                    color: colors.dark, 
                    border: 'none', 
                    borderRadius: '8px', 
                    padding: '14px', 
                ...
                >
                  {isSubmitting ? "Saving..." : <><Save size={18} /> Save Package</>}
                </button>
              </div>
            </div>
          </div>
