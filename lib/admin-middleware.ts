...
interface AdminCheckResult {
  ...
  error?: string;
  admin?: {
    // Changed from 'user' to 'admin' to match your API route
    id: string;
    email?: string;
    name?: string;
    role?: string;
...
export async function checkAdminAccess(
  ...
  try {
    ...
    console.log(
      "✅ BYPASS MODE: Granting admin access to:",
      decodedToken.email,
    );
    console.log(
      "⚠️ WARNING: This is temporary - add proper admin check later!",
    );
...
export async function checkAdminAccessBypass(
  req: NextRequest,
): Promise<AdminCheckResult> {
